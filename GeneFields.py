...
def doEverything0():
	a=geneFields(readFiles())
	if not os.path.exists('geneFieldsOP'):
    		os.makedirs('geneFieldsOP')
	os.chdir('geneFieldsOP')
	print('1.2 writeing GeneFields')
	for k in a:
		for k2 in a[k]:
			temp = open(str(k2)+'.csv',"w")
			temp.write(prDict2(a[k][k2]))
			temp.close()
			temp = open(str(k2)+'.m',"w")
			temp.write(octPrt(a[k][k2],str(k2)))
			temp.close()
